...
<html>
<head>
<title> Veterinario | Home :: w3layouts</title>
...
<div class="cols-grids panel-widget">
	...
	<h2>Veterinarios</h2>
		...
		<form id="formularioV" name="formularioC" method="post" action="agregar">
				...
				<input id="horario" name="horario" type="text" class="form-control" data-error="Ingrese una dirección" required>
				<div class="help-block with-errors"></div>
	...
	<hr>
	<div class="tabla">
	<table id="dg" title="Información Veterinarios" style="width:1024px;height:250px"
        url="veterinario" toolbar="#toolbar" pagination="true" rownumbers="true" fitColumns="true" singleSelect="true">
        <thead>
            <tr>
			    ...
			    <th field="direccion">Horario</th>
			    <th field="correo">Correo Electronico</th>
			    <th field="contra">Contraseña</th>
			    <!--<th field="codMascota">Código Mascota</th>-->
	        </tr>
        </thead>		
		<?php foreach($listVeterinario as $veterinario): ?>
		<?php
			echo '<tr class="fila"><td class="cedula">'.$veterinario['cedula'].'</td><td class="nombre">'.$veterinario['nombres'].'</td><td class="apellido">'.$veterinario['apellidos'].'</td><td class="telefono">'.$veterinario['telefono'].'</td><td class="horario">'.$veterinario['horario'].'</td><td class="correo">'.$veterinario['correo'].'</td><td class="contra">'.$veterinario['contra'].'</td></tr>';?>
		<?php endforeach; ?>
    </table>
    </div>
</section>
...
<script type="text/javascript">
	...
	function doSearch(){
		Object.get = function(obj) {
		    ...
		    for (key in obj) {
			size[i]={cedula:obj[i].cedula,nombres:obj[i].nombres,apellidos:obj[i].apellidos,telefono:obj[i].telefono,horario:obj[i].horario,correo:obj[i].correo};
			i++;
		    }
		    return size;
		};
		$.post( "listarFiltroVeterinario", { key: valor=$('#cedula').val() })
		  .done(function( data ) {
		  var res = data.split("----");
		  var info = new Object();
		  for(i=0; i<res.length-1; i++){
			var res2 = res[i].split("::::");
			info[i]={"cedula":res2[0],"nombres":res2[1],"apellidos":res2[2],"telefono":res2[3],"horario":res2[4],"correo":res2[5]};
		  }
	...
	$(".fila").click(function(e){ //boton
		 e.preventDefault();
		  ...
		  var horario=$(this).find(".horario").html();
