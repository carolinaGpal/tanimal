...
<html>
<head>
<title> Secretario | Home :: w3layouts</title>
...
<div class="cols-grids panel-widget">
	...
	<hr>
	<div class="tabla">
	<table id="dg" title="Información secretarios" style="width:1024px;height:250px"
        url="secretario" toolbar="#toolbar" pagination="true" rownumbers="true" fitColumns="true" singleSelect="true">
        <thead>
            <tr>
		...
		<?php foreach($listSecretario as $secretario): ?>
		<?php
			echo '<tr class="fila"><td class="cedula">'.$secretario['cedula'].'</td><td class="nombre">'.$secretario['nombres'].'</td><td class="apellido">'.$secretario['apellidos'].'</td><td class="telefono">'.$secretario['telefono'].'</td><td class="correo">'.$secretario['correo'].'</td><td class="contra">'.$secretario['contra'].'</td></tr>';?>
		<?php endforeach; ?>
    </table>
    </div>
</section>
    ...
    <div class="sidebar-menu">
		     ...
		     <div class="menu">
		      <ul id="menu" >
		        ...
		        <li><a href="../../index.php/ClieCd/cliente"><i class="fa fa-user-plus"></i><span>secretario</span><span></span></a></li>
...
<script type="text/javascript">
	...
	function buscar(){
		Object.get = function(obj) {
		    ...
		    for (key in obj) {
			size[i]={cedula:obj[i].cedula,nombres:obj[i].nombres,apellidos:obj[i].apellidos,telefono:obj[i].telefono,correo:obj[i].correo};
			i++;
		    }
		    return size;
		};
		$.post( "listarFiltroSecretario", { key: valor=$('#cedula').val() })
		  .done(function( data ) {
		  var res = data.split("----");
		  var info = new Object();
		  for(i=0; i<res.length-1; i++){
			var res2 = res[i].split("::::");
			info[i]={"cedula":res2[0],"nombres":res2[1],"apellidos":res2[2],"telefono":res2[3],"correo":res2[4]};
		  }
